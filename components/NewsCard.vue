<template>
    <div 
        ...
        <div
            :class="{ 'flex flex-col gap-5 md:grid grid-cols-12 md:gap-8 p-6 md:p-7' : isFeed }"
        >
            <div v-if="isFeed"
        ...
        <div
            ...
            <p class="card-normal-text text-gray-md">{{ pubDate(item.pubDate) }}</p>
        </div>
    </div>
</template>
...
const isFeed = computed(() => props.disp === "feed")
const isGrid = computed(() => props.disp === "grid")

const sourceURL = computed(() => new URL(props.item.link).hostname)
const pubDate = (date: string) => computed(() => moment(date).format("DD.MM.YYYY"));
</script>
